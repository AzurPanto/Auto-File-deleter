...
def delete_folder():
    ...
    if folder_path:
        try:
            delete_datetime = datetime.now() + timedelta(**
                                                         {time_unit_text: frequency})
            result_label.config(
                text=f"Contents of folder '{folder_path}' will be deleted every {frequency} {time_unit_text}. Next deletion on {delete_datetime}.")

            # Schedule the deletion of the folder contents
            scheduled_job = schedule.every(frequency).minutes.do(
                delete_folder_contents, folder_path)
        except FileNotFoundError:
            result_label.config(text=f"Folder '{folder_path}' not found.")
        except PermissionError:
            result_label.config(
                text=f"Permission denied to delete '{folder_path}'")
...
def delete_folder_contents(folder_path):
    try:
        ...
        result_label.config(
            text=f"Contents of folder '{folder_path}' have been deleted.")
    except FileNotFoundError:
        result_label.config(text=f"Folder '{folder_path}' not found.")
    except PermissionError:
        result_label.config(
...
delete_frequency_label = ttk.Label(
...
time_unit_combobox = ttk.Combobox(root, textvariable=time_unit, values=[
                                  "minutes", "hours", "days", "weeks"], state="readonly")
